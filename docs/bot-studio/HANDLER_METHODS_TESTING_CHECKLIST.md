...
curl -H "access-token: $ACCESS_TOKEN" \
     -H "client: $CLIENT" \
     -H "uid: $UID" \
     "https://mac-studio.tail367da4.ts.net/api/v1/accounts/$ACCOUNT_ID/agent_bots/$BOT_ID/handler_methods" \
     | jq
...
curl -H "access-token: $ACCESS_TOKEN" \
     -H "client: $CLIENT" \
     -H "uid: $UID" \
     "https://mac-studio.tail367da4.ts.net/api/v1/accounts/$ACCOUNT_ID/agent_bots/$BOT_ID/handler_methods/search?q=welcome" \
     | jq
...
curl -X POST \
     ...
     "https://mac-studio.tail367da4.ts.net/api/v1/accounts/$ACCOUNT_ID/agent_bots/$BOT_ID/handler_methods/validate" \
     | jq
```
...
1. **Navigate to Bot Studio**:
   - Go to https://mac-studio.tail367da4.ts.net/app/accounts/1/settings/agent-bots/15/studio
