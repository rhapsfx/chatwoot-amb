...
if stats[:issues].any?
  ...
  stats[:issues].first(10).each_with_index do |issue, index|
    ...
    if issue[:camelcase_fields]
      puts "  camelCase: #{issue[:camelcase_fields].join(', ')}"
    end

    if issue[:snake_case_fields]
      puts "  snake_case: #{issue[:snake_case_fields].first(5).join(', ')}"
    end
