...
# Script to create the Guitar Information Collection Form in Chatwoot
# This version uses the CORRECT structure for SendMessageService
# Usage: rails runner script/create_guitar_info_form_template.rb --account-id 1 --inbox-id 6
#
# FIXES APPLIED (learned through debugging):
#
# 1. CONTENT FIELD (BotRendererService requirement)
#    - Added 'content' field to apple_message_content
#    - BotRendererService.render_from_metadata requires this field
...
#    - BotRendererService.detect_content_type_from_attributes detects 'apple_form'
...
#    - Added 'title' and 'description' at root level of content_attributes
#    - Added 'show_summary' to ALLOWED_APPLE_FORM_KEYS
...
#    - Uses 'picker' item_type with 'picker_type: date'
#    - SendMessageService.build_msp_page_from_item supports 'picker'
#    - Converts to Apple MSP datePicker format
#
# 5. IMAGE IDENTIFIERS (Critical fix)
#    - Uses STRING identifiers (e.g., 'guitar_form_gibson') NOT numeric IDs
#    - SendMessageService.load_form_images looks up by string identifier
#    - Image identifiers included in select page items and messages
#
# 6. PAYLOAD VALIDATION (PayloadValidatorService requirement)
#    - PayloadValidator reads pages from dynamic.data.pages
#    - Images array added to interactiveData.data.images
#    - All pages converted correctly with imageIdentifier preserved
...
OptionParser.new do |opts|
  opts.banner = 'Usage: rails runner script/create_guitar_info_form_template.rb [options]'
...
content_attributes = {
  ...
  'received_message' => {
    ...
    'style' => 'small',
    'image_identifier' => 'guitar_form_header'            # Use STRING identifier
  },
  'reply_message' => {
    'title' => 'Thank You!',
          ...
          'options' => [
            {
              ...
              'imageIdentifier' => 'guitar_form_gibson'  # Use STRING identifier, not numeric ID
            },
            {
              'id' => 'martin',
              'title' => 'Martin DC28E Dreadnought',
              'value' => 'Martin DC28E Dreadnought',
              'imageIdentifier' => 'guitar_form_martin'  # Use STRING identifier, not numeric ID
            },
            {
              'id' => 'prs',
              'title' => 'Paul Reed Smith Custom',
              'value' => 'Paul Reed Smith Custom',
              'imageIdentifier' => 'guitar_form_prs'     # Use STRING identifier, not numeric ID
            }
          ]
        }
      ]
...
puts "\n✨ Success! Guitar Information Form template created with ALL fixes applied"
puts "\n📋 Template Details:"
puts "  ID: #{template.id}"
...
puts '  ✅ received_message & reply_message with image_identifier'
puts '  ✅ pages array with items'
puts '  ✅ STRING image identifiers (not numeric IDs)'
puts "\n📄 Form Pages:"
...
puts "\n🎨 Images:"
puts '  - Guitar options: guitar_form_gibson, guitar_form_martin, guitar_form_prs'
puts '  - Messages: guitar_form_header (received & reply)'
puts "\n🔧 Fixes Applied:"
...
puts '  6. String image identifiers ✅'
puts '  7. Image loading in SendMessageService ✅'
puts "\n🚀 Ready to use!"
puts "   Template ID: #{template.id}"
puts '   Use in n8n Bot API or Chatwoot ReplyBox'
puts "\n💡 Converted MSP Pages (what Apple will receive):"
puts '   1. Guitar Model (select page with images)'
