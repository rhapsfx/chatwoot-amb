...
PLAYWRIGHT_DIR="services/playwright-scraper"
...
cleanup_stale_processes() {
    ...
    # Kill any Playwright scraper processes that might be orphaned
    local playwright_pids=$(pgrep -f "playwright-scraper/server.js" 2>/dev/null || true)
    if [ -n "$playwright_pids" ]; then
        print_status "Killing stale Playwright scraper processes: $playwright_pids"
...
start_playwright() {
    if is_running "$PLAYWRIGHT_PID_FILE"; then
        print_warning "Playwright scraper is already running (PID: $(cat $PLAYWRIGHT_PID_FILE))"
        return
    fi

    if [ ! -f "$PLAYWRIGHT_DIR/server.js" ]; then
        print_warning "Playwright scraper not found at $PLAYWRIGHT_DIR/server.js — skipping"
        return
    fi

    print_status "Starting Playwright scraper (port $PLAYWRIGHT_PORT)..."
    mkdir -p log

    nohup node "$PLAYWRIGHT_DIR/server.js" > log/playwright.log 2>&1 &
    PLAYWRIGHT_PID=$!
    echo $PLAYWRIGHT_PID > "$PLAYWRIGHT_PID_FILE"

    sleep 2
    if is_running "$PLAYWRIGHT_PID_FILE"; then
        print_success "Playwright scraper started (PID: $PLAYWRIGHT_PID)"
    else
        print_error "Failed to start Playwright scraper"
        print_status "Check log/playwright.log for details"
        rm -f "$PLAYWRIGHT_PID_FILE"
    fi
}

# Function to stop Playwright scraper
stop_playwright() {
    local quiet_mode=${1:-false}
    if is_running "$PLAYWRIGHT_PID_FILE"; then
        local pid=$(cat "$PLAYWRIGHT_PID_FILE")
        print_status "Stopping Playwright scraper (PID: $pid)..."
        kill "$pid"
        rm -f "$PLAYWRIGHT_PID_FILE"
        print_success "Playwright scraper stopped"
    elif [ "$quiet_mode" != "true" ]; then
        print_warning "Playwright scraper is not running"
    fi
}
...
show_status() {
    ...
    if is_running "$PLAYWRIGHT_PID_FILE"; then
        ...
        echo -e "Playwright Scraper: ${RED}STOPPED${NC}"
    fi
