# Check and fix Acoustic House Bot template status
...
if needs_fix == 0
  ...
  puts ''
  puts "   Since templates still don't appear in UI, checking other fields..."
  puts ''

  # Check all fields
  templates.each do |t|
    puts "Template: #{t.name}"
    puts "  ID: #{t.id}"
    puts "  Account ID: #{t.account_id}"
