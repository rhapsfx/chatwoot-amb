...
export default {
  ...
  setup(props, { emit }) {
    ...
    const createApplePayRequest = () => {
      return {
        ...
        requiredBillingContactFields:
          props.paymentRequest.required_billing_contact_fields || [],
        requiredShippingContactFields:
          props.paymentRequest.required_shipping_contact_fields || [],
    ...
    const handleValidateMerchant = async event => {
      try {
        const response = await fetch(
          `/apple_messages_for_business/${props.mspId}/payment_gateway/merchant_session`,
          {
            method: 'POST',
            headers: {
        ...
        applePaySession.value.completeMerchantValidation(
    ...
    const handleShippingMethodSelected = async event => {
      try {
        const response = await fetch(
          `/apple_messages_for_business/${props.mspId}/payment_gateway/method_update`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        applePaySession.value.completeShippingMethodSelection({
          ...
          newLineItems:
            updateData.line_items || props.paymentRequest.line_items,
        });
      } catch (error) {
        applePaySession.value.completeShippingMethodSelection({
    ...
    const handlePaymentAuthorized = async event => {
      try {
        const response = await fetch(
          `/apple_messages_for_business/${props.mspId}/payment_gateway/process_payment`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
    ...
    const initiatePayment = async () => {
      if (!canPay.value) {
        errorMessage.value = t(
          'APPLE_MESSAGES.PAYMENT.APPLE_PAY_NOT_AVAILABLE'
        );
        return;
      }
    ...
    onMounted(() => {
      if (typeof window !== 'undefined' && window.ApplePaySession) {
        ...
        errorMessage.value = t(
          'APPLE_MESSAGES.PAYMENT.APPLE_PAY_NOT_SUPPORTED'
        );
      }
    });
    ...
    <div class="payment-container">
      <div class="payment-header">
        <div class="payment-icon">
          <i class="i-ph-credit-card-duotone text-2xl" />
        </div>
      ...
      <div v-if="showDetails" class="payment-items">
        <div
      ...
      <div class="payment-actions">
        ...
        >
          <div class="apple-pay-logo">
            ...
            <span class="pay-text">{{ $t('APPLE_MESSAGES.PAYMENT.PAY') }}</span>
          </div>
        </button>
      ...
      <div class="payment-details-toggle">
        <button class="details-button" @click="showDetails = !showDetails">
          <span>{{
            showDetails
              ? $t('APPLE_MESSAGES.PAYMENT.HIDE_DETAILS')
              : $t('APPLE_MESSAGES.PAYMENT.SHOW_DETAILS')
          }}</span>
          <i :class="showDetails ? 'i-ph-caret-up' : 'i-ph-caret-down'" />
        </button>
      </div>
      ...
      <div v-if="paymentResult" class="payment-success">
        ...
        {{
          $t('APPLE_MESSAGES.PAYMENT.SUCCESS', {
        ...
        <div class="transaction-id">
          {{
            $t('APPLE_MESSAGES.PAYMENT.TRANSACTION_ID', {
