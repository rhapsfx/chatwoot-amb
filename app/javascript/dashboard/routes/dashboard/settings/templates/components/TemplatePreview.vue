...
const renderPreview = async () => {
  if (!props.template.id) {
    useAlert(t('TEMPLATES.BUILDER.PREVIEW.SAVE_FIRST'));
    return;
  }
      ...
      <div class="bg-n-slate-3 px-4 py-3 border-b border-n-slate-7">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-medium text-n-slate-12">
            {{
              t('TEMPLATES.BUILDER.PREVIEW.CHANNEL_PREVIEW', {
                channel: getChannelLabel(selectedChannel),
              })
            }}
          </span>
        </div>
      ...
      <div class="bg-n-solid-1 dark:bg-n-slate-2 p-6 min-h-96">
        <div v-if="loading" class="flex items-center justify-center h-64">
          <div class="text-n-slate-10">
            {{ t('TEMPLATES.BUILDER.PREVIEW.LOADING') }}
          </div>
        </div>
        ...
        <div v-else-if="!template.id" class="text-center py-12">
          ...
          <p class="text-sm text-n-slate-11">
            {{ t('TEMPLATES.BUILDER.PREVIEW.SAVE_FIRST') }}
          </p>
        </div>
        ...
        <div v-else-if="!renderedPreview" class="text-center py-12">
          ...
          <Button icon="i-lucide-play" @click="renderPreview">
            {{ t('TEMPLATES.BUILDER.PREVIEW.GENERATE') }}
          </Button>
        </div>
          ...
          >
            ...
            >
              ...
              <div class="flex gap-2 flex-wrap">
                ...
                >
                  ...
                  {{ t('TEMPLATES.BUILDER.PREVIEW.TIME_SLOT', { n: i }) }}
                </div>
              </div>
            </div>
            ...
            <div v-else-if="block.blockType === 'payment'" class="space-y-3">
              <div class="flex items-center gap-2">
                ...
                <span class="font-medium text-n-slate-12">{{
                  t('TEMPLATES.BUILDER.PREVIEW.PAYMENT_REQUEST')
                }}</span>
              </div>
              <div class="space-y-1">
                <div class="text-sm text-n-slate-11">
                  {{ t('TEMPLATES.BUILDER.PREVIEW.MERCHANT') }}
              ...
              <div class="pt-2">
                ...
                >
                  {{ t('TEMPLATES.BUILDER.PREVIEW.PAY_NOW') }}
                </div>
              </div>
            </div>
            ...
            <div v-else-if="block.blockType === 'form'" class="space-y-3">
              ...
              <div class="space-y-2">
                <template
                  v-if="
                    block.properties.pages && block.properties.pages.length > 0
                  "
                >
                  <div
                    v-for="(field, i) in block.properties.pages[0].items.slice(
                      0,
                      4
                    )"
                    :key="i"
                  ...
                  <div
                    v-if="block.properties.pages[0].items.length > 4"
                    class="text-xs text-n-slate-11 italic"
                  >
                    {{
                      t('TEMPLATES.BUILDER.PREVIEW.MORE_FIELDS', {
                        count: block.properties.pages[0].items.length - 4,
                      })
                    }}
                  </div>
                </template>
                <div v-else class="text-sm text-n-slate-11 italic">
                  {{ t('TEMPLATES.BUILDER.PREVIEW.NO_FORM_FIELDS') }}
                </div>
              </div>
            </div>
            ...
            <div v-else class="text-sm text-n-slate-11">
              {{
                t('TEMPLATES.BUILDER.PREVIEW.BLOCK_PREVIEW', {
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="text-sm text-n-blue-11">
          <p class="font-medium mb-1">
            {{ t('TEMPLATES.BUILDER.PREVIEW.NOTE_TITLE') }}
          </p>
          <p>{{ t('TEMPLATES.BUILDER.PREVIEW.NOTE_BODY') }}</p>
        </div>
      </div>
    </div>
  </div>
