  ...
  try {
    ...
    timePickerTemplates.value = (response.data.templates || []).filter(
      template => {
        ...
        if (template.template_type === 'time_picker') return true;

        const content = template.content || {};
        const attrs =
          content.contentAttributes || content.content_attributes || {};

        // Time pickers have event with timeslots (be more lenient with structure check)
        const hasEventWithTimeslots =
          attrs.event !== undefined && attrs.event?.timeslots !== undefined;

        // Also check if it's tagged as scheduling/time picker
        const isSchedulingTemplate =
          template.category === 'scheduling' ||
          template.tags?.includes('time_picker') ||
          template.tags?.includes('scheduling');

        return hasEventWithTimeslots || isSchedulingTemplate;
      }
    );
...
const selectedTemplate = computed(() => {
  ...
  return timePickerTemplates.value.find(
