class ContentAttributeValidator < ActiveModel::Validator
  ...
  ALLOWED_APPLE_FORM_KEYS = [:title, :description, :request_identifier, :fields, :pages, :submit_url, :method, :validation_rules, :images, :received_message,
                             :reply_message, :version, :form_id, :use_live_layout, :submit_button, :cancel_button, :show_summary, :external_error, :interactive_data,
                             :form].freeze
  ...
  def validate_apple_pay!(record)
    ...
    # Skip validation for incoming interactive messages (they contain interactive_data)
    # These are validated by Apple and we just store the payment response
    return if content_attrs['interactive_data'].present?
