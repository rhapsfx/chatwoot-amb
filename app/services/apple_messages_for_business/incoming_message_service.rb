class AppleMessagesForBusiness::IncomingMessageService
  ...
  def perform
    ...
    set_conversation
    create_message
    return if @duplicate_suppressed
  ...
  def create_message
    ...
    # For IDR messages, Apple retries send a completely new IDR envelope (different id, signature,
    # key, and download URL) but the 'owner' field in interactiveDataRef is the stable blob
    # identifier — it is identical across all retries for the same user selection.
    idr_ref = @params['interactiveDataRef']
    message_source_id = if idr_ref.present? && idr_ref['owner'].present?
                          "idr:#{idr_ref['owner']}"
                        else
                          @params['id']
                        end
    Rails.logger.info "[AMB IncomingMessage] Dedup key: #{message_source_id.inspect}"
    if message_source_id.present?
      # IDR dedup is time-windowed: the same owner blob is reused for every response to the same
      # list-picker template, so a global unique constraint would suppress legitimate new selections.
      # Apple retries always arrive within a few seconds; 30 s is a safe window.
      dedup_scope = @conversation.messages.where(source_id: message_source_id)
      dedup_scope = dedup_scope.where('created_at > ?', 30.seconds.ago) if message_source_id.start_with?('idr:')
      existing_message = dedup_scope.first
      if existing_message
        Rails.logger.info "[AMB IncomingMessage] Duplicate message suppressed - source_id: #{message_source_id}, existing message_id: #{existing_message.id}"
  ...
  def extract_from_nskeyed_archiver(interactive_data)
    ...
    if params['replyMessage']&.first
      begin
        ...
        # Skip Apple's standard list-picker UI hint ("Tap this message to view your selection")
        # — it is not user-selected content and produces confusing transcript text.
        apple_hint_pattern = /\btap this message to view your selection\b/i
        content_parts = []
  ...
  def extract_content_from_interactive_data(interactive_data)
    ...
    if interactive_data['data'] && interactive_data['data']['replyMessage']
      ...
      # Use the title as the main content; skip Apple's "Tap this message..." UI hint from subtitle
      apple_hint_pattern = /\btap this message to view your selection\b/i
      content_parts = []
      content_parts << reply_message['title'] if reply_message['title'].present?
      subtitle = reply_message['subtitle']
  ...
  def process_interactive_data_reference
    # If this message was already fully processed (e.g. duplicate IDR webhook dedup hit an
    # existing record that had already been through this method), there is nothing to do.
    if @message.content_attributes['idr_processed']
      Rails.logger.info '[AMB IncomingMessage] IDR already processed for this message, skipping re-processing'
      return
    end
