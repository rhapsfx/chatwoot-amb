...
class AppleMessagesForBusiness::AcousticHouseBotService
  ...
  def process_interactive_response(interactive_data)
    ...
    request_id = if data['quick-reply']
                   ...
                   # UUID requestIdentifiers (produced by valid_or_generated_request_identifier) won't
                   # match handler names directly — fall back to state inference when there's no hit.
                   if INTERACTIVE_HANDLERS[req_id]
                     req_id
                   else
                     inferred = state_inferred_handler_id
                     log_info "[Bot] 🎯 UUID requestId not in handlers, inferred from state #{@bot_state}: #{inferred}"
                     inferred
                   end
                 else
                   ...
                   req_id = state_inferred_handler_id
  ...
  # Maps the current bot state to a handler ID when the requestIdentifier in the Apple
  # response is a UUID (generated by valid_or_generated_request_identifier) rather than
  # the original handler name string. The AHB1 form-response case is intentionally omitted
  # here because form submissions arrive via NSKeyedArchiver (no requestIdentifier) and are
  # handled separately before this method is ever called.
  def state_inferred_handler_id
    case @bot_state
    when 'AHA2' then 'qr_travel'
    when 'AHB2' then 'qr_name'
    when 'AHC1' then 'lp_guitar_0319'
    when 'AHD1' then 'qr_view_ar'
    when 'AHE1' then 'qr_place_ar'
    when 'AHG2' then 'lp_store_selection'
    when 'AHH1' then 'time_0319'
    when 'AHI1' then 'qr_continue'
    when 'AHJ1' then 'qr_photo'
    when 'AHK1' then 'qr_learn_more'
    when 'AHF1' then 'applepay_1018'
    when 'DEMO_MODE' then 'applepay_1018'
    when 'DEMO_MODE_LARGE_FORM' then 'form_large_content'
    else
      log_warn "[Bot] ⚠️ No state inference available for bot state: #{@bot_state}"
      nil
    end
  end
  ...
  def handle_store_selection_qr(interactive_data)
    ...
    selected_index = if interactive_data['$archiver'] == 'NSKeyedArchiver'
                       ...
                       # Standard quick reply format — selectedIndex is the position in available_stores
                       quick_reply_data = interactive_data.dig('data', 'quick-reply') || {}
                       quick_reply_data['selectedIndex']
                     end
