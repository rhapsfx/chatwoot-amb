...
module Templates
  class FromAppleMessageService
    ...
    def convert_quick_reply_properties
      # Frontend sends 'items' for quick replies, convert to 'replies'
      items = message_data[:items] || message_data[:replies] || []

      {
        text: message_data[:text] || message_data[:title] || message_data[:summary_text],
        subtitle: message_data[:subtitle],
        replies: items.map do |item|
          {
            title: item[:title] || item['title'],
            value: item[:value] || item['value'] || item[:title] || item['title'],
