#!/bin/bash
# test-claude-agents-argument-parsing.sh - Tests argument parsing and validation functions
# 
# Purpose: Test the parse_arguments and validate_arguments functions for claude-agents.sh
# Dependencies: None (sources claude-agents.sh directly)
...
source "$(dirname "$0")/../../scripts/claude-agents.sh"
...
test_validate_arguments_format_validation() {
    # Test format validation for list command
    local valid_formats=("compact" "detailed")
    
    for format in "${valid_formats[@]}"; do
        local input=$'command	list\nformat	'"$format"
        assert_command_succeeds "Format '$format' should be valid for list command" -- validate_arguments_with_input "$input"
    done
    
    # Test invalid format
    local input_invalid=$'command	list\nformat	invalid'
    local output
    output=$(assert_command_fails 1 "Invalid format should fail validation" -- validate_arguments_with_input "$input_invalid" 2>&1)
    assert_contains "Invalid --format for 'list' command: invalid" "$output" "Should output invalid format error"
}

test_validate_arguments_format_restriction() {
    # Test --format flag only valid for list command
    local input_invalid=$'command	install\nformat	detailed'
    local output
    output=$(assert_command_fails 1 "Format flag should not be valid for install command" -- validate_arguments_with_input "$input_invalid" 2>&1)
    assert_contains "--format option only valid for 'list' command" "$output" "Should output format restriction error"
}
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    run_tests_with_args "claude-agents.sh Argument Parsing Unit Tests" "$@"
fi